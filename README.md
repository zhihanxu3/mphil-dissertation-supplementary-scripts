...
`PlanetScope_tiles_creation.py` is the python script that split original, large PlanetScope imagery covering the entire AOI into smaller tiles of the size 4096 by 4096 pixels, in order to facilitate more manageable processing and analysis in later supervised classification works, and to ensure consistency with normal deep learning U-Net model requriement for input data size, though nnU-Net model used in this study has no such strict requirement for data dimensiosn as it is entirely self-adaptive.

splits a large PlanetScope GeoTIFF covering an entire area of interest into 4096 × 4096 pixel tiles. Tiling is beneficial for both producing more manageable image units for subsequent annotation and supervised classification, and meeting the fixed input size requirements typical of standard deep learning segmentation models. Though the nnU-Net model used in this study does not require a particular input size, as it automatically adapts to the image, later study may find this step useful is employing other deep learning models.
